...
- Rules — path-scoped coding standards (src/, ai/, tests/, src/game/)
...
- ~~เพิ่มใน core/01 folder structure~~ ✅ ai/07-decisions/entity-register.md
...
read_more:
  ...
  game_design: ai/08-design/fdd-index.md
  source_current: ai/00-source/versions/v1.2/
-->
```
...
**สิ่งที่ต้องสร้าง**
- ~~template ไฟล์ `ai/03-log/agents/[tool].md` ใน core/08~~ ✅
...
**workflow**
```
1. โปรเจ็กต์ใหม่ bootstrap → index ai/ เข้า shared store เป็น project index ใหม่
2. AI session เริ่ม → ค้น store ก่อน: "โปรเจ็กต์ไหนเคยแก้ปัญหาคล้ายนี้"
3. พบ match → แสดง pattern + link ไปโปรเจ็กต์ต้นทาง
4. session end → sync ai/ ที่เปลี่ยนกลับเข้า store
```
...
**สิ่งที่ต้องสร้าง**
- `tools/sync-projects.sh` — sync ai/ เข้า shared store หลัง session end
