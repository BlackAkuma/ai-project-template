...
โครงสร้าง doc/ ของเราแบ่ง folder ตาม concern อยู่แล้ว — เหมาะกับการ index เป็น section แยกกัน

**การ map doc/ เข้า search index**
```
doc/00-source/       ← source docs (immutable, versioned)
doc/01-plan/         ← project plan + work status
doc/02-task/         ← task board
doc/03-log/          ← session logs + agent diaries
doc/07-decisions/    ← ADR + entity register
```

**วิธี integrate**
- script ที่ index doc/ เข้า vector store ที่เลือก (ChromaDB, Qdrant, หรืออื่น)
...
**สิ่งที่ต้องสร้าง**
- script `tools/index-docs.sh` — index doc/ เข้า vector store ที่เลือก
...
**workflow**
```
1. โปรเจ็กต์ใหม่ bootstrap → index doc/ เข้า shared store เป็น project index ใหม่
2. AI session เริ่ม → ค้น store ก่อน: "โปรเจ็กต์ไหนเคยแก้ปัญหาคล้ายนี้"
3. พบ match → แสดง pattern + link ไปโปรเจ็กต์ต้นทาง
4. session end → sync doc/ ที่เปลี่ยนกลับเข้า store
```
...
**สิ่งที่ต้องสร้าง**
- `tools/sync-projects.sh` — sync doc/ เข้า shared store หลัง session end
