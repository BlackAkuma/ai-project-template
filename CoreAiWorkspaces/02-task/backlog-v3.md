...
|----|-----|---------------|----------------|
| **DEV-FP** | forward-port T-060 enforcement → dev (reconcile กับ govern-action engine hook กัน hook ซ้อน) | dogfood divergence | dev เท่านั้น |
| **FU-1** | reject re-open flow (เปลี่ยนใจ in-band ได้) | P0 re-review | engine |
| **FU-2** | inbox.jsonl file-lock/atomic — **บังคับก่อน Phase B multi-agent** | P0 re-review | engine |
