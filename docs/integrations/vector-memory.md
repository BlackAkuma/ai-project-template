...
### ขั้น 2: Index ไฟล์ใน CoreAiWorkspaces/

```bash
# รูปแบบทั่วไป:
[tool] index CoreAiWorkspaces/ --wing <project-name>

# ตัวอย่าง (แทน [tool] ด้วย tool จริงที่ใช้):
[tool] index CoreAiWorkspaces/ --wing my-ecommerce-project
```
...
### จบ session — ถ้า CoreAiWorkspaces/ มีการเปลี่ยนแปลง
...
| เมื่อ | Action |
|-------|--------|
| จบ session (ถ้า CoreAiWorkspaces/ เปลี่ยน) | `[tool] index CoreAiWorkspaces/ --wing <project-name>` |
| เพิ่มไฟล์ใหม่ใน CoreAiWorkspaces/ | `[tool] index CoreAiWorkspaces/ --wing <project-name>` |
