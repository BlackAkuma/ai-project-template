...
ทำ Scenario M checklist ก่อนแตะโค้ดทุกครั้ง (→ `CoreAiWorkspaces/04-way-of-work/ai-decision-protocol.md`)

### กฎ 9 ข้อ (บังคับ)

| # | กฎ | AI ทำ |
|---|----|-------|
| 1 | ก่อนเริ่มทุก task → ตรวจ branch + status + ค้าง feature | `git status` + `git branch --list "feature/*" "fix/*"` |
| 2 | งานเล็ก ≤2 ไฟล์ → ทำบน dev ได้ | ไม่ต้องสร้าง branch |
| 3 | งานใหญ่ → สร้าง branch จาก dev ก่อนแตะโค้ด | `git checkout -b feature/<task-id>-<name>` |
| 4 | Commit + push ทุก milestone (ไม่ค้าง > 1 task) | `git push origin <branch>` |
| 5 | เสร็จ → merge --no-ff → ลบ branch ทั้ง local + remote | `git branch -d` + `git push --delete origin <branch>` |
| 6 | One feature at a time — ห้ามมี active feature branch >1 | ตรวจ `git branch --list` ก่อนสร้างใหม่ |
| 7 | Promotion (dev→sit→uat→main) → user สั่งเท่านั้น | รอ explicit command |
| 8 | Session End → push เสมอ (WIP OK) | บังคับ push ก่อนจบ |
| 9 | ไม่แน่ใจ → แยก branch ไว้ก่อน | กฎ safe-default |

### Decision Tree: ต้องแยก branch ไหม?

```
ต้องแยก   : Feature ใหม่ / Architecture change / ADR-level / Cross-module / ≥3 ไฟล์
ทำบน dev  : Bug fix 1 จุด / Doc edit / Config tweak / ≤2 ไฟล์
ไม่แน่ใจ  : แยกไว้ก่อน (กฎ 9)
```

### Branch Naming

```
feature/<task-id>-<name>    เช่น  feature/T-012-auth-module
fix/<task-id>-<name>        เช่น  fix/T-015-login-crash
chore/<name>                เช่น  chore/update-deps
```
