...
if [ -d "$TARGET/ai" ]; then
  echo "ERROR: $TARGET/ai มีอยู่แล้ว — ไม่ overwrite"
  exit 1
fi
...
dirs=(
  "$TARGET/ai/00-source/versions/v0.1"
  "$TARGET/ai/01-plan"
  "$TARGET/ai/02-task"
  "$TARGET/ai/03-log"
  "$TARGET/ai/04-way-of-work"
  "$TARGET/ai/05-summary/$YEAR/$MONTH"
  "$TARGET/ai/06-extensions"
  "$TARGET/ai/07-decisions"
)
if [ $IS_GAME -eq 1 ]; then
  dirs+=("$TARGET/ai/08-design")
fi
...
make_file "$TARGET/ai/00-source/README.md" \
...
make_file "$TARGET/ai/00-source/versions/v0.1/README.md" \
...
make_file "$TARGET/ai/01-plan/project-plan.md" \
"# Project Plan — $PROJECT_NAME

**Source:** ai/00-source/versions/v0.1/
**Created:** $DATE
...
make_file "$TARGET/ai/01-plan/work-status.md" \
"<!-- AI-CONTEXT
phase: bootstrap
active_task: none
blocker: none
...
1. เพิ่ม source docs ใน \`ai/00-source/versions/v0.1/\`
2. กรอก project plan ใน \`ai/01-plan/project-plan.md\`
3. สร้าง task แรกใน task board"

# ── 5. 02-task/ ───────────────────────────────────────────────────────────────

make_file "$TARGET/ai/02-task/task-board.md" \
"<!-- AI-CONTEXT
total_tasks: 0
in_progress: []
blocked: []
...
make_file "$TARGET/ai/03-log/work-log-index.md" \
"<!-- AI-CONTEXT
last_session: $DATE
tool: Claude Code
completed: []
...
- สร้าง ai/ โครงสร้างทั้งหมด
- Next: เพิ่ม source docs และ task แรก"

# ── 7. 04-way-of-work/ ────────────────────────────────────────────────────────

make_file "$TARGET/ai/04-way-of-work/way-of-work.md" \
...
# ai-decision-protocol: copy from core/11, strip template meta-header, set project title
{
  ...
  tail -n +5 "$TEMPLATE_ROOT/core/11-ai-decision-protocol-template.md" \
    | sed \
        -e "s/<PROJECT_NAME>/$PROJECT_NAME/g" \
        -e "s/<CURRENT_DATE>/$DATE/g"
} > "$TARGET/ai/04-way-of-work/ai-decision-protocol.md"
echo "  created: $TARGET/ai/04-way-of-work/ai-decision-protocol.md (from core/11)"
...
# compliance: copy from core/15, strip template meta-header, strip fictional example data
{
  ...
  tail -n +2 "$TEMPLATE_ROOT/core/15-compliance-check-template.md" \
    | sed \
        ...
        -e '/src\/enemy\/ai\.ts/d'
} > "$TARGET/ai/04-way-of-work/compliance.md"
echo "  created: $TARGET/ai/04-way-of-work/compliance.md (from core/15)"

make_file "$TARGET/ai/04-way-of-work/coding-standards.md" \
...
make_file "$TARGET/ai/03-log/README.md" \
"# Log — $PROJECT_NAME
...
mkdir -p "$TARGET/ai/03-log/templates"
make_file "$TARGET/ai/03-log/templates/daily-log-template.md" \
"# Daily Log — $DATE
...
make_file "$TARGET/ai/05-summary/README.md" \
"# Summaries — $PROJECT_NAME

Monthly summaries เก็บใน \`YYYY/MM/\`
ใช้ template จาก \`templates.md\`"

make_file "$TARGET/ai/05-summary/templates.md" \
...
make_file "$TARGET/ai/06-extensions/README.md" \
...
make_file "$TARGET/ai/06-extensions/extension-template.md" \
"# Extension: <EXTENSION_TITLE>

**วันที่:** $DATE
**Source Reference:** ai/00-source/versions/v0.1/<filename>.md §<section>
**Related Tasks:** T-XXX
**Author:** <ชื่อ>
...
mkdir -p "$TARGET/ai/07-decisions"
cat > "$TARGET/ai/07-decisions/README.md" <<EOF
<!-- AI-CONTEXT
total_adrs: 0
accepted: []
proposed: []
...
EOF
echo "  created: $TARGET/ai/07-decisions/README.md (live ADR index)"

# entity-register — write live content directly (not extract from meta-template)
cat > "$TARGET/ai/07-decisions/entity-register.md" <<EOF
<!-- AI-CONTEXT
entities_active: []
entities_deprecated: []
last_updated: $DATE
...
| *(ยังไม่มี)* | | | | | | |
EOF
echo "  created: $TARGET/ai/07-decisions/entity-register.md (live entity register)"
...
if [ $IS_GAME -eq 1 ]; then
  ...
  make_file "$TARGET/ai/08-design/README.md" \
...
**กฎ:** ทุก feature ใหม่ต้องมี FDD ที่ Approved ก่อน task จะออกจาก design_validate"
  ...
  make_file "$TARGET/ai/08-design/asset-registry.md" \
"<!-- AI-CONTEXT
total_assets: 0
last_updated: $DATE
-->
...
echo ""
echo "[$([ $IS_GAME -eq 1 ] && echo '4' || echo '3')/6] Verifying output..."

REQUIRED=(
  "ai/README.md"
  "ai/00-source/README.md"
  "ai/00-source/versions/v0.1/README.md"
  "ai/01-plan/project-plan.md"
  "ai/01-plan/work-status.md"
  "ai/02-task/task-board.md"
  "ai/03-log/README.md"
  "ai/03-log/work-log-index.md"
  "ai/03-log/templates/daily-log-template.md"
  ...
  "ai/05-summary/README.md"
  "ai/05-summary/templates.md"
  "ai/06-extensions/README.md"
  "ai/06-extensions/extension-template.md"
  "ai/07-decisions/README.md"
  "ai/07-decisions/entity-register.md"
)
if [ $IS_GAME -eq 1 ]; then
  REQUIRED+=("ai/08-design/README.md" "ai/08-design/asset-registry.md")
fi
...
echo ""
echo "  Next steps:"
echo "  1. เพิ่ม source docs ใน ai/00-source/versions/v0.1/"
