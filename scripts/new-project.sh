...
if [ -d "$TARGET/doc" ]; then
  echo "ERROR: $TARGET/doc มีอยู่แล้ว — ไม่ overwrite"
  exit 1
fi
...
dirs=(
  "$TARGET/doc/00-source/versions/v0.1"
  "$TARGET/doc/01-plan"
  "$TARGET/doc/02-task"
  "$TARGET/doc/03-log"
  "$TARGET/doc/04-way-of-work"
  "$TARGET/doc/05-summary/$YEAR/$MONTH"
  "$TARGET/doc/06-extensions"
  "$TARGET/doc/07-decisions"
)
if [ $IS_GAME -eq 1 ]; then
  dirs+=("$TARGET/doc/08-design")
fi
...
make_file "$TARGET/doc/README.md" \
"# doc/ — $PROJECT_NAME
...
make_file "$TARGET/doc/00-source/README.md" \
...
make_file "$TARGET/doc/00-source/versions/v0.1/README.md" \
...
make_file "$TARGET/doc/01-plan/project-plan.md" \
"# Project Plan — $PROJECT_NAME

**Source:** doc/00-source/versions/v0.1/
**Created:** $DATE
...
make_file "$TARGET/doc/01-plan/work-status.md" \
"<!-- AI-CONTEXT
phase: bootstrap
active_task: none
blocker: none
...
make_file "$TARGET/doc/02-task/task-board.md" \
"<!-- AI-CONTEXT
total_tasks: 0
in_progress: []
blocked: []
...
make_file "$TARGET/doc/03-log/work-log-index.md" \
"<!-- AI-CONTEXT
last_session: $DATE
tool: Claude Code
completed: []
...
- สร้าง doc/ โครงสร้างทั้งหมด
- Next: เพิ่ม source docs และ task แรก"

# ── 7. 04-way-of-work/ ────────────────────────────────────────────────────────

make_file "$TARGET/doc/04-way-of-work/way-of-work.md" \
...
# ai-decision-protocol: copy from core/11, strip template meta-header, set project title
{
  ...
  tail -n +5 "$TEMPLATE_ROOT/core/11-ai-decision-protocol-template.md" \
    | sed \
        -e "s/<PROJECT_NAME>/$PROJECT_NAME/g" \
        -e "s/<CURRENT_DATE>/$DATE/g"
} > "$TARGET/doc/04-way-of-work/ai-decision-protocol.md"
echo "  created: $TARGET/doc/04-way-of-work/ai-decision-protocol.md (from core/11)"
...
# compliance: copy from core/15, strip template meta-header, strip fictional example data
{
  ...
  tail -n +2 "$TEMPLATE_ROOT/core/15-compliance-check-template.md" \
    | sed \
        ...
        -e '/src\/enemy\/ai\.ts/d'
} > "$TARGET/doc/04-way-of-work/compliance.md"
echo "  created: $TARGET/doc/04-way-of-work/compliance.md (from core/15)"

make_file "$TARGET/doc/04-way-of-work/coding-standards.md" \
...
make_file "$TARGET/doc/03-log/README.md" \
"# Log — $PROJECT_NAME
...
mkdir -p "$TARGET/doc/03-log/templates"
make_file "$TARGET/doc/03-log/templates/daily-log-template.md" \
"# Daily Log — $DATE
...
make_file "$TARGET/doc/05-summary/README.md" \
"# Summaries — $PROJECT_NAME

Monthly summaries เก็บใน \`YYYY/MM/\`
ใช้ template จาก \`templates.md\`"

make_file "$TARGET/doc/05-summary/templates.md" \
...
make_file "$TARGET/doc/06-extensions/README.md" \
...
make_file "$TARGET/doc/06-extensions/extension-template.md" \
"# Extension: <EXTENSION_TITLE>

**วันที่:** $DATE
**Source Reference:** doc/00-source/versions/v0.1/<filename>.md §<section>
**Related Tasks:** T-XXX
**Author:** <ชื่อ>
...
mkdir -p "$TARGET/doc/07-decisions"
cat > "$TARGET/doc/07-decisions/README.md" <<EOF
<!-- AI-CONTEXT
total_adrs: 0
accepted: []
proposed: []
...
EOF
echo "  created: $TARGET/doc/07-decisions/README.md (live ADR index)"

# entity-register — write live content directly (not extract from meta-template)
cat > "$TARGET/doc/07-decisions/entity-register.md" <<EOF
<!-- AI-CONTEXT
entities_active: []
entities_deprecated: []
last_updated: $DATE
...
| *(ยังไม่มี)* | | | | | | |
EOF
echo "  created: $TARGET/doc/07-decisions/entity-register.md (live entity register)"
...
if [ $IS_GAME -eq 1 ]; then
  ...
  make_file "$TARGET/doc/08-design/README.md" \
...
**กฎ:** ทุก feature ใหม่ต้องมี FDD ที่ Approved ก่อน task จะออกจาก design_validate"
  ...
  make_file "$TARGET/doc/08-design/asset-registry.md" \
"<!-- AI-CONTEXT
total_assets: 0
last_updated: $DATE
-->
...
REQUIRED=(
  "doc/README.md"
  "doc/00-source/README.md"
  "doc/00-source/versions/v0.1/README.md"
  "doc/01-plan/project-plan.md"
  "doc/01-plan/work-status.md"
  "doc/02-task/task-board.md"
  "doc/03-log/README.md"
  ...
  "doc/05-summary/README.md"
  "doc/05-summary/templates.md"
  "doc/06-extensions/README.md"
  "doc/06-extensions/extension-template.md"
  "doc/07-decisions/README.md"
  "doc/07-decisions/entity-register.md"
)
if [ $IS_GAME -eq 1 ]; then
  REQUIRED+=("doc/08-design/README.md" "doc/08-design/asset-registry.md")
fi
...
echo "  Path: $TARGET/doc/"
echo ""
echo "  Next steps:"
echo "  1. เพิ่ม source docs ใน doc/00-source/versions/v0.1/"
