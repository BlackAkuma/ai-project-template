...
| **`+ platforms/claude-code/`** | Claude Code CLI users | CLAUDE.md auto-loaded · Commit hooks · Path-scoped rules · /caw-session-end · /caw-adr-create |
...
```bash
# 1. Download ZIP and extract to _template/ in your project
mkdir my-project && cd my-project
curl -L https://github.com/BlackAkuma/ai-project-template/archive/master.zip -o _t.zip
unzip _t.zip && mv ai-project-template-master _template && rm _t.zip

# 2. Run bootstrap script — installs everything automatically
bash _template/scripts/new-project.sh "My Project Name" .
...
- `CLAUDE.md` at root — Claude Code reads this every session
- `.claude/commands/` — slash commands `/caw-session-end`, `/caw-adr-create`, etc.
- `.git/hooks/validate-commit` — checks deprecated entities before commit
- `CoreAiWorkspaces/` — AI working folder with full structure

Open Claude Code — **every session loads automatically, no prompts needed**.
...
Project details:
- Name: [PROJECT_NAME]
- Path to create CoreAiWorkspaces/ at: [PROJECT_ROOT_PATH]
...
4. Create the CoreAiWorkspaces/ structure at the path above
...
Once AI confirms the bootstrap checklist passes — delete the template folder. What remains is `CoreAiWorkspaces/` in your project (and `.claude/` if using Claude Code).
...
| `01` Folder Structure | Standard `CoreAiWorkspaces/` directory layout |
...
|----------|-------------|
| First setup (core 19 files + build CoreAiWorkspaces/) | ~18,000 |
...
| **`+ platforms/claude-code/`** | Claude Code CLI users | CLAUDE.md โหลดอัตโนมัติ · Commit hooks · Rules ตาม path · /caw-session-end · /caw-adr-create |
...
```bash
# 1. ดาวน์โหลด ZIP แล้วแตกไว้ใน _template/ ของโปรเจ็กต์
mkdir my-project && cd my-project
curl -L https://github.com/BlackAkuma/ai-project-template/archive/master.zip -o _t.zip
unzip _t.zip && mv ai-project-template-master _template && rm _t.zip

# 2. รัน bootstrap script — ติดตั้งทุกอย่างอัตโนมัติ
bash _template/scripts/new-project.sh "ชื่อโปรเจ็กต์" .

# 3. ลบ template folder
rm -rf _template/
```

Script ติดตั้งให้อัตโนมัติ:
- `CLAUDE.md` ที่ root — Claude Code อ่านทุก session
- `.claude/commands/` — slash commands `/caw-session-end`, `/caw-adr-create` ฯลฯ
- `.git/hooks/validate-commit` — ตรวจ deprecated entities ก่อน commit
- `CoreAiWorkspaces/` — AI working folder พร้อมโครงสร้างครบ

เปิด Claude Code — **ทุก session โหลดอัตโนมัติ ไม่ต้อง prompt ซ้ำ**
...
ข้อมูลโปรเจ็กต์:
- ชื่อ: [PROJECT_NAME]
- Path ที่จะสร้าง CoreAiWorkspaces/: [PROJECT_ROOT_PATH]
...
4. สร้างโครงสร้าง CoreAiWorkspaces/ ที่ path ด้านบน
...
เมื่อ AI ยืนยันว่า bootstrap checklist ผ่านแล้ว — ลบ template folder ทิ้ง เหลือแค่ `CoreAiWorkspaces/` ในโปรเจ็กต์ (และ `.claude/` ถ้าใช้ Claude Code)
...
| `01` Folder Structure | โครงสร้าง `CoreAiWorkspaces/` มาตรฐาน |
...
|-----------|-------------|
| Setup ครั้งแรก (core 19 ไฟล์ + สร้าง CoreAiWorkspaces/) | ~18,000 |
| Setup + game skill pack | ~23,000 |
