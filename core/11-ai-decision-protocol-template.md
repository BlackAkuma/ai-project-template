...
0. ⛔ challenge necessity — ทำก่อนทุกขั้น ไม่ข้าม
   ...
   จากนั้นรัน 3-lens internal challenge (ทำใน response เดียว ไม่ต้อง spawn):

   🔍 Expert lens   — proposal นี้แก้ปัญหาอะไร? มี pattern ที่คล้ายกันใน system แล้วไหม?
                      ถ้ามีอยู่แล้ว: ระบุว่าอยู่ที่ไหน — ไม่เสนอซ้ำ
   ⚙️ Technical lens — complexity เท่าไหร่? reversible ไหม? มี dependency อะไร?
                      ถ้า complexity สูงกว่าประโยชน์: ระบุ + เสนอทางที่ lean กว่า
   🔴 Contrarian lens — เหตุผลที่แข็งแกร่งที่สุดที่ไม่ควรทำสิ่งนี้คืออะไร?
                       (บังคับ — ห้ามตอบว่า "ไม่มีข้อโต้แย้ง")

   → ถ้า 3 lenses เห็นว่าควรทำ: สรุปสั้น ๆ แล้วดำเนินต่อขั้น 1–7
   → ถ้ามี lens ใดคัดค้านมีเหตุผล: หยุดก่อน แสดง synthesis ให้ผู้ใช้ตัดสินใจ
...
7. scope check — สำคัญที่สุด
   → task นี้เพิ่ม entity / mechanic / concept ใหม่ที่ไม่มีใน source docs หรือ project plan?
   → ถ้าใช่: SCOPE CHANGE — หยุด แจ้งผู้ใช้ ขออนุมัติแยก (ดู Scenario N)
...
### Scenario N — ผู้ใช้บอก "ทำต่อ" / "ได้เลย" / "ทำตามที่แนะนำ"

เกิดเมื่อ: ผู้ใช้ให้ continuation approval ระหว่างทำงาน

**"ทำต่อ" / "ได้เลย" = approve เฉพาะ task/milestone ปัจจุบันที่กำลัง discuss เท่านั้น**  
ไม่ใช่ blanket approval สำหรับ arc ทั้งหมดที่ AI เสนอไว้

```
ก่อน implement milestone ถัดไปทุกครั้ง ถามตัวเองก่อน:

1. task นี้อยู่ใน scope ที่ approved แล้วไหม?
   → ถ้าใช่: ทำได้เลย

2. task นี้เพิ่ม entity / mechanic / concept ใหม่ที่ไม่มีใน source docs หรือ project plan?
   → ถ้าใช่: SCOPE CHANGE — หยุดทันที ต้องขออนุมัติแยก ห้ามใช้ "ทำต่อ" เก่าเป็นข้ออ้าง

3. task นี้เป็น milestone ใหม่ในระหว่างที่ผู้ใช้ไม่ได้เห็น plan ก่อน?
   → ถ้าใช่: บอก plan สั้น ๆ ก่อน รอยืนยัน แล้วค่อยทำ

4. ADR สำหรับ task นี้อยู่ในสถานะ Proposed (ยังไม่ Accepted)?
   → ถ้าใช่: STOP — ห้าม implement จนกว่าผู้ใช้จะ Accept ADR
```

**กฎ:** 1 "ทำต่อ" = 1 task/milestone เท่านั้น  
ถ้า arc มี 5 milestone → ต้องได้รับ approval 5 ครั้ง ไม่ใช่ 1 ครั้งตอนต้น
...
| **Level 2 — Block & Flag** | ผลกระทบปานกลาง ไม่แน่ใจ | mark task blocked, อัปเดต work-status, รอมนุษย์ | A, B, E, F, H, J, N |
