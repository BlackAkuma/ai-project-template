...
- `ai/README.md`
- `ai/00-source/`
- `ai/00-source/versions/<CURRENT_SOURCE_VERSION>/`
...
- `ai/04-way-of-work/ai-decision-protocol.md` (จาก `11-ai-decision-protocol-template.md`)
- `ai/04-way-of-work/compliance.md` (จาก `15-compliance-check-template.md`)
- `ai/05-summary/`
- `ai/06-extensions/`
- `ai/07-decisions/README.md` (ADR index จาก `12-adr-template.md`)
- `ai/07-decisions/entity-register.md` (จาก `17-entity-register-template.md`)
...
- `ai/` และ `CLAUDE.md` ต้องไม่ขึ้น production branch — นี่คือ core principle
- daily logs และ daily summaries อาจเป็น local working records และไม่จำเป็นต้องเก็บใน git
- เมื่อเจอสถานการณ์คลุมเครือหรือขัดแย้ง ให้ปฏิบัติตาม `ai/04-way-of-work/ai-decision-protocol.md`
- ก่อนตัดสินใจเชิง architecture ต้องอ่าน `ai/07-decisions/README.md` ก่อน
...
3. **โหลด skill pack** ตามคำตอบ:
   - Game / Web Game → อ่าน `skills/game/` ทั้งหมดเพิ่มเติมจาก core → สร้าง `ai/08-design/` ด้วย
   - อื่น ๆ → ใช้ core เท่านั้น
   - **การตรวจสอบ session ถัดไป:** ถ้าโปรเจ็กต์มี `ai/08-design/` แปลว่าเป็น game project — โหลด game skill standards อัตโนมัติ
4. อ่าน source docs ที่มีอยู่
5. สร้าง `ai/` ตาม template folder structure (เพิ่ม `ai/08-design/` ถ้าเป็น game)
6. สร้าง source docs index และ version folders
7. วางเอกสารปฏิบัติงานตั้งต้นจาก core templates
8. ถ้าเป็น game → วาง game skill templates เพิ่มเติมใน `ai/04-way-of-work/` และ `ai/08-design/`
...
- ไม่ commit `ai/` หรือ `CLAUDE.md` ลงบน production branch โดยไม่แจ้งผู้ใช้
